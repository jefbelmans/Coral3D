...
bool vkutil::load_image_from_file(VulkanEngine& engine, const char* file, AllocatedImage& outImage)
{
	...
	stbi_uc* img{ stbi_load(file, &tex_width, &tex_height, &tex_channels, STBI_rgb_alpha) };
	...
	AllocatedBuffer staging_buffer{ engine.create_buffer(img_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY) };

	// Copy data to buffer
	void* data;
	vmaMapMemory(engine.m_Allocator, staging_buffer.allocation, &data);
	...
	vmaCreateImage(engine.m_Allocator, &image_ci, &image_ai,
	...
	engine.m_MainDeletionQueue.deletors.emplace_back([=]()
		{
			vmaDestroyImage(engine.m_Allocator, new_img.image, new_img.allocation);
		});
